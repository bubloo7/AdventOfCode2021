...
for line in arr:
    ...
    if nums[0] == nums[2] or nums[1] == nums[3]:
        hor_or_ver.append([(int(nums[0]),  int(nums[1])),
...
for tup in edges:
    xdif = int((tup[0][0] - tup[1][0])/abs((tup[0][0] - tup[1][0])))*-1
    ydif = int((tup[0][1] - tup[1][1])/abs((tup[0][1] - tup[1][1])))*-1
    count = int(abs(tup[0][1] - tup[1][1]))
    for h in range(count + 1):
        floor[tup[0][1] + (h * ydif)][tup[0][0] + (h * xdif)] += 1
